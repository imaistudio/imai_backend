...
async def upscale_image(
    ...
    """Upscale an image using FAL AI's AuraSR"""
    try:
        # Create directories
        create_directories()
        ...
        try:
            ...
            upscaled_path = Path('output') / f"upscaled_{Path(image_url).name}"
...
async def main_async():
    ...
    if len(sys.argv) < 2:
        print("Usage: python upscale.py [scale] [enhance_face] [enhance_details] [input_image]")
        print("Example: python upscale.py 2 true true path/to/image.jpg")
        sys.exit(1)
    ...
    enhance_face = sys.argv[2].lower() == 'true' if len(sys.argv) > 2 else True
    enhance_details = sys.argv[3].lower() == 'true' if len(sys.argv) > 3 else True
    input_image = sys.argv[4] if len(sys.argv) > 4 else None
